...
func New(db *sql.DB, conName string, interval, timeout time.Duration, labels map[string]string) *Gauges {
...
func (g *Gauges) observeOnce(gauge prometheus.Gauge, query string, params []interface{}) {
	log.WithField("db", g.name).Debugf("collecting")
	var result []float64
